# %%
# 多頭注意力 (Multi-Head Attention) 時間序列預測
# 透過 Self-Attention 對過去 OHLCV 序列進行建模，預測下一日收盤價。
...
class Model(nn.Module):
    def __init__(self, input_size, attn_embed_dim, attn_num_heads, dropout, output_size):
        super(Model, self).__init__()
        self.linear1 = nn.Linear(input_size, attn_embed_dim)
        ...
        self.linear2 = nn.Linear(attn_embed_dim, output_size)

    def forward(self, inputs):
        # 線性投影到注意力空間
        outputs = self.linear1(inputs)
        # Self-Attention：對序列內各時間步之間做關聯建模
...
# %%
# 設定輸出路徑（TensorBoard 與模型參數），統一放在 outputs/{時間戳}/attention。

RUN_ID = get_run_id()
ATTN_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "outputs", RUN_ID, "attention")
WRITER_PATH = os.path.join(ATTN_OUTPUT_DIR, "writer")
MODELPARAM_PATH = os.path.join(ATTN_OUTPUT_DIR, "model_param")
PLOTS_PATH = os.path.join(ATTN_OUTPUT_DIR, "plots")
...
plot_path = os.path.join(PLOTS_PATH, "attention_test_prediction.png")

lineplot_true_and_predicted_result(
    true_values=trues,
    predicted_values=preds,
    title="Attention Model Test Set Prediction",
    save_path=plot_path,
)
...
if __name__ == "__main__":
    # 直接執行此檔案即可完成 Attention 模型訓練與測試流程。
    pass
