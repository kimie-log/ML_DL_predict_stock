# %%
# XGBoost 範例
# 使用 XGBoost 進行隔日漲跌方向（二分類），並視覺化特徵重要性。
# 注意：執行前請先安裝 xgboost，且在 macOS 需額外安裝 libomp（OpenMP runtime）。
...
bst = xgboost.train(params=params, dtrain=dtrain, evals=evals)
...
y_valid_pred = (valid_proba > 0.5).astype(int)
